...
while [ $userguess != $secretnumber ]; do # ask repeatedly until they get it right
  [ -v "$userguess" ] && echo "User input in not empty"
  read -p "Give me a number from 1 to 10: " userguess # ask for another guess
  [ $userguess -gt 0 -a $userguess -lt 10 ] && echo "User input is in range" || echo "User input not in range"
  [ $userguess -lt $secretnumber ] && echo "Your guess is too low"
  [ $userguess -gt $secretnumber ] && echo "Your guess is too high"
done
